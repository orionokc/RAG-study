...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    #load file from temporary location using pdf loader
    loader = PyMuPDFLoader(temp_file.name)
    docs = loader.load()
    #os.unlink(temp_file.name) #Delete temp file
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=400, #how many characters to split document with into chunks
        chunk_overlap=100, #how much overlap between chunks
        separators=["\n\n", "\n", ".", "?", "!", " ", ""] #paragraph, senteces, questions, exclamation marks
    )
...
def get_vector_collection() -> chromadb.Collection:
    ...
    return chroma_client.get_or_create_collection(
        ...
        metadata={'hnsw:space':'cosine'}, #could also use Euclidean (L2), Manhattan (L1), or Dot Product
    )
...
if __name__ == "__main__":
    #side bar to upload notes to train on
    with st.sidebar:
        st.set_page_config(page_title="Smart Notes")
        st.header("Smart Notes 💡")
        uploaded_file = st.file_uploader("** Upload your Notes (PDF)!**", type=['pdf'], accept_multiple_files=False)
    ...
    st.header("🗣️ Ask your notes!")
    prompt = st.text_area("Ask your notes a question:")
    ask = st.button("Ask🔎")
    if ask and prompt:
